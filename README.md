...
- **界面字体**（两个 face 按 `unicode-range` 分工，互不重叠）：
  - 汉字 → `theme/loli-cjk/loli-cjk.woff2`（Heiti TC Light 改版的 CJK 子集，21,112 码位、4.06 MB）
  - 拉丁 → `theme/zenmarugothic/ZenMaruGothic-Medium.woff2`（Zen Maru Gothic Medium 的拉丁子集，346 码位、16 KB；原 TTF 3.6 MB）
  - 两者构建时拷到站点根目录同源提供，`@font-face` 与字体栈写在 `theme/site-overrides.css`；粗体交给浏览器合成，代码区保留官方等宽字体。
  - ⚠️ **loli 不具备再分发授权**：其原始字体版权归 Changzhou SinoType / Apple（`All rights reserved`），文件本身是第三方改版且 license 字段已被抹去。随本仓库提供是站点所有者知情后的决定，**不代表可以自由转用**；对外分发或换站前请换成 OFL 类中文字体（霞鹜文楷、思源黑体等）。
  - Zen Maru Gothic 是日文字体（汉字为日式写法，GSUB 无 `locl` 变体，`lang` 也切不出来），所以只让它负责拉丁。
